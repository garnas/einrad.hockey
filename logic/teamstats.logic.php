...
// Teste, ob ueberhaupt Daten vorhanden sind
$eval = true;
$spiele = $stats->get_anzahl_spiele();
if ($spiele == 0) $eval = false;
$turniere = $stats->get_anzahl_turniere();
if ($turniere == 0) $eval = false;

// Gehe weiter, wenn Daten vorhanden sind
if ($eval) {
    ...
    $hoechster_sieg_substring = is_null($hoechster_sieg) ? "-" : TeamRepository::get()->team($hoechster_sieg['team_id_b'])->getName();
    ...
    $hoechste_niederlage_substring = is_null($hoechste_niederlage) ? "-" : TeamRepository::get()->team($hoechste_niederlage['team_id_b'])->getName();

    $bestes_turnier = $stats->get_bestes_turnier();
    if (is_null($bestes_turnier)) {
        $bestes_turnier_string = "-";
        $bestes_turnier_subtext = "-";
    } else {
        $bestes_turnier_string = $bestes_turnier['ergebnis'];
    ...
    if (is_null($schlechtestes_turnier)) {
        $schlechtestes_turnier_string = "-";
        $schlechtestes_turnier_subtext = "-";
    } else {

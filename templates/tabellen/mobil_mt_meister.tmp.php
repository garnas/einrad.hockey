...
<?php if ($meisterschafts_tabelle_meister): ?>
    <!-- Zeilen der Meisterschaftstabelle -->
    <div>
        <?php $counter = 0; ?>
        <?php foreach ($meisterschafts_tabelle_meister as $key => $zeile): ?>
            ...
            <div onclick="show_small_results('meister', <?=$key?>)" style="cursor: pointer" id="small-meister-head-<?=$key?>" class="w3-row <?=$nthcolor?> w3-border-bottom w3-border-grey w3-hover-text-secondary">
                ...
                <div class="w3-col w3-right w3-padding-8 w3-center" style="width: 42px;">
                    <span>
                        ...
                        <span id="small-meister-icon-hide-<?=$key?>" class="material-icons w3-text-primary" style="display:none">arrow_drop_up</span>
                    </span>
                </div>
            ...
            <?php if (!empty($zeile['details'])): ?>
                <?php foreach ($zeile['details'] as $ergebnis): ?>
                    <a href="ergebnisse.php?saison=<?= $saison ?>#<?= $ergebnis['turnier_id'] ?>" class="no">
                        <div class="small-meister-result-<?= $key ?> w3-row w3-text-primary <?= $nthcolor ?> w3-hover-text-secondary"
                             style="display: none;">

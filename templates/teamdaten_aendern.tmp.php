...
<form class="w3-card w3-panel" method="post" enctype="multipart/form-data">
    <?php if (empty($daten['teamfoto'])) { ?>
        <p><i>Es können Bilder im <b>.jpg, .jpeg, .gif, .png</b> Format mit bis zu 11,9 Megabyte hochgeladen werden.
                Bilder werden webtauglich verarbeitet - exif-Daten der Bilder werden gelöscht.</i></p>
        <p>
    ...
    <?php } else { ?>
        <p>
            <img src="<?= $daten['teamfoto'] ?>" alt="<?= $daten['teamname'] ?>" class="w3-card w3-image"
    ...
    <?php }  //end if?>
    <p class="w3-text-grey">Das Teamfoto kann
        öffentlich <?= Form::link('../liga/teams.php#' . $daten['team_id'], 'auf der Teams-Seite') ?> eingesehen
        werden.</p>
</form>
...
<form method='post'>
    ...
    <div class="w3-panel w3-card">
        <p>
            ...
            <input class='w3-input w3-border w3-border-primary' type='text' id='ligavertreter' name='ligavertreter'
                   required value='<?= $daten['ligavertreter'] ?>'>
            <span class="w3-text-grey"><i>Nur eine Person kann als Ligavertreter angegeben werden</i></span>
        </p>
        <p>
            <input type="checkbox" <?php if (!empty($daten['ligavertreter'])){ ?>checked<?php } //endif?>
                   class="w3-check" value="zugestimmt" name="dsgvo" id="dsgvo">
            <label for="dsgvo" class="" style="cursor: pointer">Der Ligavertreter hat
                die <?= Form::link(Config::LINK_DSGVO, "Datenschutz-Hinweise") ?> gelesen und ihnen zugestimmt.</label>
        </p>
        <p>
            <label for='plz' class="w3-text-primary">PLZ</label>
            <input class='w3-input w3-border w3-border-primary' type='number' name='plz' name='plz'
                   value='<?= $daten['plz'] ?>'>
        </p>
        <p>
            <label for='ort' class="w3-text-primary">Ort</label>
            <input class='w3-input w3-border w3-border-primary' type='text' id='ort' name='ort'
                   value='<?= $daten['ort'] ?>'>
        </p>
        <p>
            <label for='verein' class="w3-text-primary">Verein</label>
            <input class='w3-input w3-border w3-border-primary' type='text' id='verein' name='verein'
                   value='<?= $daten['verein'] ?>'>
        </p>
        <p>
            <label for='homepage' class="w3-text-primary">Homepage</label>
            <input class='w3-input w3-border w3-border-primary' type='url' id='homepage' name='homepage'
                   value='<?= $daten['homepage'] ?>'>
        </p>
        <div class="w3-responsive w3-card">
            <table class="w3-table w3-striped">
                ...
                <?php foreach ($akt_team_kontakte->get_emails_with_details() as $email){ ?>
                    <tr>
                        <td style='vertical-align: middle'><?= $email['email'] ?></td>
                        <td class='w3-center'>
                            <select style='max-width: 100px' class='w3-select w3-border w3-border-primary'
                                    name='public<?= $email['teams_kontakt_id'] ?>'>
                                <option value='Ja'>Ja</option>
                                <option value='Nein' <?= ($email['public'] === 'Nein') ? 'selected' : '' ?> >Nein</option>
                            </select>
                        </td>
                        <td class='w3-center'>
                            <select style='max-width: 100px' class='w3-select w3-border w3-border-primary'
                                ...
                                <option value='Ja'>Ja</option>
                                <option value='Nein' <?= ($email['get_info_mail'] === 'Nein') ? 'selected' : '' ?> >Nein</option>
                            </select>
                        </td>
                        <td class='w3-center'>
                            <select style='max-width: 100px' class=' w3-select w3-border w3-border-primary'
...
<form method='post'>
    ...
    <p class=""><b>Öffentlich*</b>: Deine Email-Adresse wird in der
        Teamsliste <?= Form::link('../liga/teams.php', 'hier') ?> angezeigt. Andere Ligateams können nicht-öffentliche
        Emails auf der Webseite nicht einsehen, dafür aber im Kontaktcenter anschreiben.</p>
    <p class=""><b>Infomails**</b>: Du bekommst automatische Infomails, z. B. wenn ein für dein Team relevantes Turnier
        eingestellt wird.</p>
</form>
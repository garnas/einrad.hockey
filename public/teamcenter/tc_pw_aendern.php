...
if(isset($_POST['change'])) {
    ...
    if (strlen($passwort_neu) < 6) {
        $error = true;
        Form::error("Euer neues Passwort muss mindestens 6 Zeichen lang sein");
    }
...
<form method="post" class="w3-panel w3-card-4">
    ...
    <label class="w3-text-grey" for="passwort_alt"><?= Form::icon('remove') ?> Altes Passwort:</label>
    <input required
    ...
    >
    <p>
    <label class="w3-text-grey" for="passwort_neu"><?= Form::icon('add') ?> Neues Passwort:</label>
    <input required
           class="w3-input w3-border w3-border-primary"
           type="password"
           id="passwort_neu"
    ...
    <p>
        <button class="w3-button w3-tertiary" type="submit" name="change">
            <?= Form::icon('create') ?> Passwort ändern
        </button>
    </p>
</form>

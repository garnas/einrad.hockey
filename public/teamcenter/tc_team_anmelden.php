...
if (strtotime($akt_turnier->daten['datum']) < Config::time_offset()){
...
if ($akt_turnier->daten['art'] == 'spass'){
    $kontakt = new Kontakt ($akt_turnier->daten['ausrichter']);
    $email = $kontakt->get_emails();
    Form::attention("Anmeldung zu Spass-Turnieren erfolgt über den Ausrichter: " . Form::mailto($email, Team::teamid_to_teamname($akt_turnier->daten['ausrichter'])));
...
$abmelden_moeglich_bis = LigaBot::time_offen_melde($akt_turnier->daten['datum']) + 2*7*24*60*60;
...
if (isset($_POST['anmelden'])){
    $error = false;
    if (!($akt_turnier->daten['art'] == 'I' or $akt_turnier->daten['art'] == 'II' or $akt_turnier->daten['art'] == 'III')){
    ...
    */
    if (!$error){
        ...
        if ($akt_turnier->daten['phase'] == 'offen'){
            $liste='melde';
        }
...
if (isset($_POST['freilos'])){
    ...
    if ($akt_turnier->daten['phase'] == 'melde' && $akt_turnier->check_team_block($_SESSION['team_id'])){
...
if (isset($_POST['abmelden']) && isset($_SESSION['team_id'])){
    $error = false;
    $liste = $akt_turnier->get_team_liste($_SESSION['team_id']);
    if (empty($liste)){
        $error = true;
    ...
    if (!$error){
        if ($liste == 'warte' && $akt_turnier->daten['phase'] != 'offen'){
            $warteliste_neu = true;
        }else{
        ...
        if ($akt_turnier->anzahl_freie_plaetze() > 0 && $akt_turnier->daten['phase'] == 'melde'){
            $akt_turnier->spieleliste_auffuellen();
        }
        if ($warteliste_neu){
            $akt_turnier->warteliste_aktualisieren();
        }
        $akt_turnier->schreibe_log("Abmeldung: ". $_SESSION['teamname'] ."\r\nvon Liste: " . $liste, $_SESSION['teamname']);
        header('Location: ../teamcenter/tc_team_anmelden.php?turnier_id=' . $akt_turnier->daten['turnier_id']);
        die();
    }
}
...
if (isset($_POST['bewerben'])){
    ...
    if (!$error){
        ...
        $akt_turnier->schreibe_log("Bewerbung für Spielplatz:\r\n". $_SESSION['teamname'] ."\r\nauf Liste: melde", $_SESSION['teamname']);
        header('Location: ../teamcenter/tc_team_anmelden.php?turnier_id=' . $akt_turnier->daten['turnier_id']);
        die();
    }
}
...
<h3 class="w3-text-grey"><?=$akt_turnier->daten['tname'] ?: 'Turnier'?> in <?=$akt_turnier->daten['ort']?>, <?=strftime("%d.%m.%Y (%A)", strtotime($akt_turnier->daten['datum']))?> (<?=$akt_turnier->daten['tblock']?>)</h3>

<a href='../liga/turnier_details.php?turnier_id=<?=$akt_turnier->daten['turnier_id']?>' class="w3-text-hover-secondary w3-text-blue no">
    <i class="material-icons">keyboard_arrow_left</i>Turnierdetails
</a>
<a href='../teamcenter/tc_turnierliste_anmelden.php?turnier_id=<?=$akt_turnier->daten['turnier_id']?>' class="w3-right w3-text-hover-secondary w3-text-blue no">
...
<div class="w3-card w3-container">
    <form method='post'>
        ...
        <p>Freie Plätze: <?=$akt_turnier->daten['plaetze'] - count(($anmeldungen['spiele'] ?? array()))?> von <?=$akt_turnier->daten['plaetze']?></p>
        <p class="w3-small w3-text-primary">Phase: <?=$akt_turnier->daten['phase']?></p>
    </form>
